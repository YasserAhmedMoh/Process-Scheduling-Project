...
{
    public partial class Nonpre_priority : Form
    {
        private object is_completed;

        public Nonpre_priority()
        {
            InitializeComponent();
        }

        private void Nonpre_priority_Load(object sender, EventArgs e)
        {
            create_p();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            foreach (Control c in groupBox3.Controls)
            {
                if (c is TextBox)
                {
                    ...
                    if (txt.Text != "")
                    {
                        ...
                        counter1++;
                        /*starting_time[counter] = starting_time[counter - 1] + int.Parse(c.Text);
                        end_time[counter] = starting_time[counter] + int.Parse(c.Text);
                        waiting_time[counter] = starting_time[counter];
                        total_wait += waiting_time[counter];
                        counter++;*/
                    }
                }
            }
            ...
            /*processes[0].waiting_time[0] = 0;
            processes[0].starting_time[0] = arrival_time[0];
            end_time[0] =arrival_time[0]+burst_time[0];*/
            int current_time = 0;
            int completed = 0;
            int prev = 0;
           ...
           /* int min_arrival_time = 10000000;
            ...
            for (int i = 0; i < MaxProsNum; i++)
            {
                min_arrival_time = Math.Min(min_arrival_time, processes[i].arrival_time);
                max_completion_time = Math.Max(max_completion_time, processes[i].completion_time);
            }
           */
